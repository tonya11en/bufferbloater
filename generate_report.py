...
ax2.set_ylabel('RPS')
ax2.plot(rq_rate_x1,rq_rate_y1, '--')
ax2.plot(rq_rate_x2,rq_rate_y2, '--')
...
if len(timeout_stamps1 + timeout_stamps2) > 0:
    ...
    ax3.hist([timeout_stamps1, timeout_stamps2], bins=1000, density=True, histtype='bar', stacked=True, range=(0,relative_sim_end))
...
if len(service_unavail_stamps1 + service_unavail_stamps2) > 0:
    ...
    ax4.hist([service_unavail_stamps1,service_unavail_stamps2], bins=1000, density=True, histtype='bar',
            stacked=True, range=(0,relative_sim_end), color=["green", "blue"])
...
ax6.plot(rq_sr_x1, rq_sr_y1, '-')
ax6.plot(rq_sr_x2, rq_sr_y2, '-')
